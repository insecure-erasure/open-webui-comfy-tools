...
Image tools additionally share an **image gallery**: opening any image's lightbox (fullscreen) shows ‹ › buttons, a "n/N" counter (bottom-right) and arrow-key navigation to walk every image generated in the conversation by the four image tools (Smart Generate Image, Edit Image, Enhance Image, Virtual Try-On), with wrap-around. Like the chat itself, only the most recent messages stay mounted in the DOM, so older images leave the gallery until you scroll up. See DESIGN.md §11.
