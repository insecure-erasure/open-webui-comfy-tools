...
- **Each tool is a self-contained module** pasted into Open WebUI
  (Workspace → Tools). It **cannot import repo modules**. The embed HTML is
  stored per-tool as `<tool>/<tool>.html` (header comment + template) and
  loaded at runtime from `CACHE_DIR/tools/<tool_id>/<tool>.html` — the same
  cache-dir convention the workflow JSONs use (see REFACTOR_EMBEDS.md). The
  `.py` builders only escape values and inject them into the template.
...
- **The image gallery (Phase 8, 2026-08-04, awaiting test)**: from any image
  ...
  `gallery=True`. 13 node cases pass.
...
## How the embed HTML is stored (post-refactor, 2026-08-05)

The embed markup lives in `<tool>/<tool>.html` (header comment + template
with `{token}` placeholders), loaded at runtime via `_load_embed(tool_id, ...)`
from `CACHE_DIR/tools/<tool_id>/<tool>.html` — the same cache-dir convention
as the workflow JSONs. The `.py` builders escape the values (URLs, prompt,
gallery/prompt attributes, aspect ratio) and inject them into the template
with `re.sub(r"\{(\w+)\...", ...)`. `embeds.py` was removed (it was only a
repo-side reference; the `.html` files are now the single source of truth).
See REFACTOR_EMBEDS.md for the full inventory and the verification harness.

Verify changes with: load the module (stub fastapi/httpx/pydantic), write the
`.html` into a fake `CACHE_DIR/tools/<tool_id>/`, call the builder, and assert
the render matches expectations (or diff against a git-HAD render).

## Gotchas that cost a lot of time (do not repeat)

## Gotchas that cost a lot of time (do not repeat)

- **The old "regenerate from `embeds.py`" flow is dead**: the viewer/video
  markup now lives in the `.html` templates; edit those directly. (Historical
  note: when the viewer was an f-string, regenerating from `embeds.py` with a
  regex, the f-string closed with THREE newlines before `def
  build_video_player` — no longer relevant.)
