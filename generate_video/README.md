...
The result renders as a **Rich UI embed** (see DESIGN.md §6): a self-contained video player (`autoplay muted loop playsinline controls`) displayed inline in the chat via a sandboxed iframe. The player sizes itself after the video metadata loads, with the height capped at 65vh (65% of the available screen height). The tool returns a bare `HTMLResponse` — it is a terminal result with an empty context, so the LLM receives the middleware's generic message. No download button (native controls only).
...
The workflow JSONs can be edited freely. You can replace the diffusion models, samplers, schedulers, or any other node with whatever works for your setup. The tool injects parameters from both the LLM call arguments (prompt, image) and the valves. Everything else uses whatever the workflow defines.
