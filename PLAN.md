...
| 2026-08-04 | 1 | Slider not draggable on mobile (phase reopened) | Switched from `mousemove` to **Pointer Events** (mouse + touch), drag anywhere or tap to position, `setPointerCapture` + `touch-action:none`. Added a narrow rectangular handle with arrows at the divider center as a visual affordance. Commit `…` |
