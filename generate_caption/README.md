...
The workflow JSON can be edited freely. You can replace the Florence-2 model with any compatible variant or add custom prompt engineering. The tool injects parameters from both the LLM call arguments (image, detail_level) and the valves. Everything else uses whatever the workflow defines.
