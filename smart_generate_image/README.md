...
- Clicking the image opens a **lightbox** that fills the browser window via the **Fullscreen API** (image fit to screen, no scroll), with an X to close (top-left) and a **download** button (top-right) that forces the download. Browsers that don't allow fullscreen (e.g. some mobile/iOS) fall back to showing it inside the embed area. The embed does not resize while in fullscreen and re-fits on close, so the chat scroll stays in place (no jump to the top when closing).
- **Gallery**: when the lightbox is open, ‹ › buttons (vertically centered), a "n/N" counter (bottom-right) and the ArrowLeft/ArrowRight keys walk **all the images generated in the conversation** by the image tools (smart_generate_image, edit_image, enhance_image, virtual_try_on), starting from the one being viewed, with wrap-around at the ends. The download button always downloads the image currently shown. The gallery only appears when the conversation has more than one generated image and requires the *Iframe Sandbox Allow Same Origin* setting ON (it walks the parent chat DOM); otherwise the lightbox behaves as before. Note: like the chat itself, only the **most recent messages** stay mounted in the DOM — older images leave the gallery until you scroll up in the chat (Open WebUI's lazy message window). See DESIGN.md §11.
- **Prompt caption**: in fullscreen only, the image shows the **prompt it was generated from** at the bottom in white, over a translucent gradient that darkens toward the bottom so the text stays readable (the thumbnail never shows it). As the gallery navigates, the caption follows the shown image's prompt. See DESIGN.md §12.
