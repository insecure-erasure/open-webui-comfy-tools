...
class Tools:
    ...
    def _build_image_viewer(self, image_url: str, aspect_ratio: tuple[int, int] | None = None, gallery: bool = False, prompt: str | None = None, tool_id: str = "") -> str:
        ...
        if aspect_ratio:
            ...
            ratio_js = "null"
        template = _load_embed(tool_id, "smart_generate_image.html")
        return re.sub(
            r"\{(\w+)\}",
    ...
    async def smart_generate_image(
        ...
        try:
            ...
            viewer = self._build_image_viewer(image_url, aspect_ratio=(reduced_w, reduced_h), gallery=True, prompt=prompt, tool_id=__id__)
            return HTMLResponse(
