...
## 1. smart_generate_image / edit_image / virtual_try_on

(Formerly also enhance_image, which was renamed to upscale_image and migrated
to the compare slider — see the phase notes.)
...
slider instead of the viewer — see §10.)*
...
- The image tools (`smart_generate_image`, `edit_image`, `virtual_try_on`) all
  build their viewer with `gallery=True`. `compare_images`, `generate_video`
  and `upscale_image` (which uses the compare slider) never use `.viewer`, so
...
images show no caption. `compare_images`/`generate_video` are excluded as always.
...
- **Aspect ratio known a priori**: `smart_generate_image` computes `reduced_w:reduced_h` and can reserve the aspect ratio to avoid the load "jump". `edit_image` and `virtual_try_on` do **not** know the output dimensions in advance (they depend on the input image), so their viewers must rely on `reportHeight()` after the image loads instead of aspect reservation. (`upscale_image`, ex-`enhance_image`, uses the compare slider, which sizes from the original image's aspect ratio.)
